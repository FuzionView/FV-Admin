module ApplicationHelper
  def new_link(text, path, hide_text='')
    link_to path, class: 'text-body' do
          content_tag(:i, '', title: text, class: 'bi-file-earmark-plus ms-2 me-2', style: 'font-size: 2rem;') +
  ...
  def view_link(text, path)
    link_to path, class: 'text-body' do
      content_tag(:span, text, class: 'visually-hidden',) +
        content_tag(:i, '', title: text, class: 'bi-eye', style: 'font-size: 2rem;')
    end
  end
  ...
  def users_link(text, path)
    link_to path, class: 'text-body' do
          content_tag(:span, text, class: 'visually-hidden',) +
          content_tag(:i, '', title: text, class: 'bi-person', style: 'font-size: 2rem;')
      end
  end
  ...
  def delete_link(text, path, msg)
    link_to path, class: 'text-body',
      data: { turbo: true, turbo_method: :delete,
          ...
          content_tag(:span, text, class: 'visually-hidden',) +
          content_tag(:i, '', title: text, class: 'bi-trash ms-2', style: 'font-size: 2rem;')
      end
  end
end
